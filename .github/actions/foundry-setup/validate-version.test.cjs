...
test('default and explicit baseline require no API or cooldown', async () => {
  for (const input of [undefined, '', BASELINE]) {
    assert.equal(await validateVersion(input, noFetch, now), BASELINE);
  }
});

test('reject floating, malformed, prerelease and injectable input before fetching', async () => {
  for (const input of ['stable', 'nightly', '^1.8.3', 'v1.8', '1.8.3', 'v01.8.3',
...
test('accept newer patch, minor and major releases at exactly 14 days', async () => {
...
test('reject releases inside cooldown, in future or with missing/invalid timestamp', async () => {
  for (const published_at of [new Date(now - COOLDOWN_MS + 1).toISOString(),
    ...
    await assert.rejects(validateVersion('v1.8.4', async v => release(v, { published_at }), now), /14 days/);
  }
});
